...
## All three "not yet verified" items above confirmed done; 3-day Resend spam re-check scheduled — 2026-09-06, continued

Each checked directly with the user, one at a time, this session: (1) Gmail's "Send mail as" list for `verymeanguy11@gmail.com` shows `contact@taiwanleads.com` with edit/delete/make-default controls and no "(verify)" tag — confirmed verified, screenshot reviewed. (2) Zoho's Forwards toggle for `contact@taiwanleads.com` → `verymeanguy11@gmail.com` confirmed ON (filled/blue switch), screenshot reviewed. (3) Real end-to-end test performed: an email sent from a genuinely separate address (`verymeanguy10@gmail.com`) to `contact@taiwanleads.com` arrived in `verymeanguy11@gmail.com` in 16 seconds; Gmail's "Show original" on it showed SPF/DKIM/DMARC all PASS. Caveat noted to the user: since that test message's own From address was gmail.com, the PASS results mainly validate Gmail's own authentication surviving Zoho's forward (via SRS-rewritten envelope sender) — they don't directly exercise taiwanleads.com's own SPF/DKIM/DMARC records the way a Resend-sent email would. The reply-as-contact@ step (sent from verymeanguy11@gmail.com using the contact@taiwanleads.com send-as identity) was also confirmed sent without bounce or error.

The one item that genuinely can't be checked yet — whether Resend-sent signup/verification emails still land in Gmail spam — needs the DMARC record to have had a few days to build domain reputation first. Scheduled a reminder (3 days out, 2026-09-09) to come back and actually re-test this, rather than leaving it to be remembered manually.

## "Finish the site fully" scoped and resolved as already met — 2026-09-06, continued

The open sequencing question from the 2026-09-05 "Plan going forward" entry ("what 'finish the site' concretely means is not yet scoped") was resolved this session, not deferred again. Researched 藍新/NewebPay's own publicly stated merchant-website standards directly (not assumed): regardless of individual vs. company tier, they require the applicant's site to show product/service info with pricing, a refund policy, customer-service contact info matching the application, and a valid real domain (not a social-media page or marketplace listing).

Checked each against the live site: pricing shown (`/pricing`) ✓; real domain (taiwanleads.com) ✓; contact info matching the application — as of this session's Zoho work, `contact@taiwanleads.com` is now a real, monitored mailbox ✓ (this was the one genuinely missing piece before today); refund policy — initially thought missing (a search of this log's own text for "refund"-type keywords found nothing), but checking the actual live `app/(marketing)/terms/page.tsx` directly found this was a mistaken conclusion from searching the wrong source: **Section 六 ("取消、解除與退款") already covers this** — cancel anytime, access continues to the paid period's end, no cooling-off/refund right under the Consumer Protection Act because the service is explicitly scoped to commercial/business use (Section 一) — and the pricing page's own footer restates this at the point of purchase. This reads as real, lawyer-reviewed content already, so no standalone refund-policy page was drafted; duplicating it risked introducing language that could conflict with the reviewed original.

**User's decision, given the above:** "finished" means meeting 藍新's own external bar (which the site already does, as of today's contact@ fix) plus a reliably working core product — not clearing every item on the internal deferred-items list (session invalidation on password reset, rate-limiting on /data-removal, un-pause UX for saved searches, the noreply/notify sender-address inconsistency, unverified-signup cleanup, etc.), which are unlikely to be anything a payment processor's merchant-review team would ever look at. **Net result: nothing further needs to be built before applying for the 藍新 individual-merchant account** — the application is unblocked as of this session.

Separately surfaced and discussed, not resolved as new work: reading `app/(marketing)/pricing/page.tsx` directly (not just this log) surfaced that, as of the 2026-09-05 "Paddle hidden from the checkout surface" entry, the live checkout buttons for both paid plans currently show `NewebpayCheckoutButton`, which fails gracefully (no `NEWEBPAY_MERCHANT_ID`/`HASH_KEY`/`HASH_IV` yet) — meaning no one can currently subscribe to any paid plan. Raised directly with the user, including whether Paddle's live (non-sandbox) mode might be a faster near-term path to real revenue in parallel with the 藍新 process. **User's direct correction: this framing was unwelcome and the underlying premise is wrong to keep revisiting** — the user has already decided against Paddle in favor of 藍新, considers Paddle's own live-approval process similarly slow (contradicting this session's own earlier 2026-09-04 research citing "near-instant, 2-4 business days" — the user's direct, current statement is treated as the more reliable data point here), and considers the site not genuinely "live" yet regardless of being publicly reachable. **Do not re-raise a Paddle-live-mode suggestion in a future session** without new information changing the picture; Paddle stays in sandbox, 藍新 is the only path being pursued. Confirmed as fact for future sessions: **Paddle has only ever run in sandbox mode on this site — zero real payments have ever been processed by either processor.**

No code files changed by this entry.

## Daily/weekly/monthly digest window had an off-by-one, inflating "last N days" to N+1 calendar days; fixed — 2026-09-06, continued

Direct user request: daily digest notifications should contain only companies registered "yesterday and the day before" — i.e., exactly two calendar days — because anything broader felt like a nuisance. Checking `lib/cadence.ts`'s `CADENCE_LOOKBACK_DAYS` (added 2026-09-05, supposedly already doing "last ~2/7/30 days") against the user's literal ask surfaced a real, pre-existing bug rather than a missing feature: the window was computed as an inclusive date range from `at - lookbackDays*24h` through `at` itself (i.e., through *today*, in whatever calendar `at`'s own `toISOString()` falls on). An inclusive range spanning exactly `lookbackDays*24h` of raw time covers `lookbackDays + 1` distinct calendar dates, not `lookbackDays` — so "daily" (2) was actually surfacing 3 days of registrations (including a same-day sliver of "today"), "weekly" (7) was surfacing 8, and "monthly" (30) was surfacing 31. Confirmed with a standalone script computing `getCadenceWindow()`'s output across several `at` values (a normal digest-cron timestamp, an arbitrary afternoon UTC time, and a year boundary) before touching the real files — each produced exactly 2/7/30 calendar days, always ending the day *before* today, never today itself.

**Fix:** new `getCadenceWindow(cadence, at)` in `lib/cadence.ts`, used by both `lib/email/digest.ts` (the query that decides what a digest email contains) and `app/api/searches/[id]/digest-export/route.ts` (the per-notification CSV download link, which must reproduce exactly what its originating email showed — same invariant as before, just now derived from one shared function instead of two independently-duplicated calculations). It anchors on the **Taiwan calendar date** `at` falls on (via `Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Taipei" })`, not UTC's date — registration_date reflects GCIS's own Taiwan-local setup dates, so the boundary should be figured in that calendar rather than relying on the coincidence that this app's cron schedule usually keeps UTC and Taipei dates aligned at the moment digest.ts runs), then builds a window of exactly `lookbackDays` calendar dates ending the day *before* that anchor date. For daily, that's exactly `[day-before-yesterday, yesterday]` — never today, matching the user's literal request and also fixing the underlying off-by-one for weekly/monthly as a side effect (same root cause, smaller relative impact since one extra day out of 7 or 30 is less noticeable than one extra day out of 2).

**Verified:** cloned the public repo into a sandbox, `npm ci`, `npx next typegen` + `npx tsc --noEmit` clean, `npx eslint` on all three touched files clean, and the standalone `getCadenceWindow()` sanity script referenced above confirmed exactly 2/7/30-day output across three different `at` values including a year boundary. The identical, already-verified edit was then applied to the real project files. **Not yet verified against a real send** — the next daily/weekly/monthly digest run (or a manual `workflow_dispatch` of `digest.yml`) is the actual confirmation that live emails now reflect this.

**Modified:** `lib/cadence.ts` (new `getCadenceWindow()` export, `CADENCE_LOOKBACK_DAYS` unchanged), `lib/email/digest.ts`, `app/api/searches/[id]/digest-export/route.ts`.

```
git add lib/cadence.ts lib/email/digest.ts "app/api/searches/[id]/digest-export/route.ts" architecture.md
...
## August ingestion shortfall investigated: 8/1-11 root cause confirmed via git history; second gap traced to two distinct, real findings — 2026-09-06, continued

The user brought their own analysis (done outside this session, not previously logged here): comparing June (3,526 / 21 workdays / 167.9 per day), July (3,506 / 23 / 152.4), and August (1,399 / 14 non-weekend-non-8/1-11 days / 99.9) — August's per-workday density is ~40% below the June/July baseline even after excluding the known 8/1-11 void and known weekend zero-days. User's read: two separate problems, not one.

**8/1-11 root cause, confirmed via git history:** `run-ingest-daily.ts` / `.github/workflows/ingest-daily.yml` — the only code capable of discovering a company by specific registration day — was first committed **2026-08-13 08:09 +0800** (`cfec971`). Before that, no code anywhere could discover a day-specific registration at all; this is a capability gap, not an outage or missed cron. The monthly bulk pipeline runs a fixed `0 3 5 * *` schedule with its own publication lag, so the Aug 5 run had no August data to find either.

**Delivered `scripts/check-august-ingestion-gap.ts` (read-only), user ran it against production. Real output surfaced two distinct findings, neither of which was the original "just waiting on the monthly dataset" hypothesis:**

**Finding 1 — the daily pipeline structurally never captures "business" (商業/個人商號) entities, only "company" (公司).** The monthly datasets split cleanly into `company_new/change/dissolve` and `business_new/change/dissolve` as genuinely separate GCIS datasets. `run-ingest-daily.ts` hardcodes `entity_type = 'company'` on every row it inserts (line ~334) and its own comment confirms business-type rows are deliberately left to "their own separate monthly backfill path already." In July, `business_new` alone was **4,905 rows versus `company_new`'s 3,507** — business-type registrations are the *majority* of new registrations (58%), not a minor category. This means every 方案C (daily-cadence, highest-paying) subscriber currently never sees a newly-registered sole proprietorship/商業 in their daily digest at all until the monthly bulk pipeline eventually catches up — which, per the ingestion_runs check below, can be weeks. **This is a real, likely more consequential product gap than the density question below, and needs a decision from the user on priority — not something that self-heals on its own on any useful timescale for a daily-cadence customer.**

**Finding 2 — even controlling for entity type correctly, the density gap is real.** The user's original June/July baseline (3,526 / 3,506) matches `company_new`-only totals almost exactly (confirmed: June `company_new+change+dissolve` = 3,526 exactly; July `company_new` alone = 3,507). August's entire count (1,399) is 100% `company`-type already (the daily pipeline's only output type), so this is already an apples-to-apples comparison — the mixing-of-entity-types theory from the previous entry does NOT explain the shortfall away. The `ingestion_runs` check also showed the Sept 5 monthly run only re-checked `source_month=2026年07月` (found `no_new_data` — July's dataset was already fully ingested, nothing new) — **no run has ever attempted an August-labeled monthly dataset at all**, consistent with August's own monthly bulk data not yet existing on data.gov.tw's side.

**Leading hypothesis for finding 2, not yet confirmed:** GCIS's live `Company_Setup_Date` API (queried by the daily pipeline the very next morning) may itself be a provisional, incomplete index that fills in more records over the following days/weeks — a real characteristic of relying on a next-day live query against a government system, distinct from the eventual authoritative monthly bulk release. **Could not test this directly** — this sandbox's network egress blocks `data.gcis.nat.gov.tw` (same restriction noted for the Prospect Directory scraper). Delivered a second standalone script, `scripts/check-gcis-live-recount.ts` (no database needed, `tsc`/`eslint` clean) — queries GCIS's live API right now for six already-known August dates (8/12, 13, 14, 20, 28, 31) and compares today's live count against what the daily pipeline actually captured on the day itself. A meaningfully higher count today would confirm the "provisional index" theory (inherent limitation, not a bug); a matching count would mean the real cause is still unidentified and needs to be looked for in the fetch/pagination code itself. **Not yet run by the user as of this entry.**

**Not addressed, deliberately deferred to whoever picks this up next:** whether/how to fix Finding 1 (give 方案C subscribers real-time visibility into business-type registrations too — would need a second live GCIS endpoint/dataset for `商業`, mirroring what `run-ingest-daily.ts` already does for `company`) is a real scoping decision, not started here pending the user's priority call on it.

No code files changed to fix anything in this entry — diagnostic only. New files: `scripts/check-august-ingestion-gap.ts`, `scripts/check-gcis-live-recount.ts`.

## GCIS live-index recount confirmed; discovered a second, more serious consequence and sized it — 2026-09-06, continued

**Finding 2 (density gap) confirmed, closed, needs no code fix.** User ran `scripts/check-gcis-live-recount.ts` against production. Result: today's live `Company_Setup_Date` count came back meaningfully higher than what the daily pipeline captured on the day, for all six test dates, with no exceptions — deltas of +70, +23, +63, +52, +58, +72 (captured average 99.8/day → live-today average 156.2/day, closely matching the June/July healthy baseline of 150-170/day). This confirms the leading hypothesis directly: GCIS's live `Company_Setup_Date` endpoint is a provisional index that keeps filling in more records for weeks after the date it describes — a real characteristic of that government API, not a bug in this codebase's fetch or pagination logic. **No fix needed for the density gap itself.**

**But that same confirmed fact has a much more serious implication, not previously considered: if GCIS's live index is still filling in weeks later, then every daily-cadence digest that queried "yesterday" on the morning after got an incomplete answer — and the ~35-40% of that day's registrations GCIS hadn't indexed yet were never queried again.** `run-ingest-daily.ts` only ever asks about "yesterday," once. Those late-arriving rows do eventually get captured — but only when the monthly bulk pipeline catches up, which the Sept 5 run showed can be weeks after the fact (see previous entry: no August monthly dataset has landed yet as of Sept 5). By the time a late-arriving row is finally ingested, it may already be too old for *any* cadence's window (`getCadenceWindow()`, added 2026-09-05: daily=2 days, weekly=7, monthly=30) — and `lib/email/digest.ts`'s existing "stale match" behavior marks a too-old match `surfaced_in_digest = true, surfaced_at = now()` **without ever emailing it**, using the exact same columns as an actually-emailed match. There is no flag distinguishing the two. So a meaningful share of real registrations could be marked "surfaced" and never actually reach any subscriber, on any cadence, permanently — a data-loss problem for the product's core promise, not merely a density/cosmetics issue. This reasoning was traced through already-read code, not yet confirmed against real production data at the time it was raised.

Flagged this to the user directly given the stakes. Asked how to proceed; user chose to size it first with one more diagnostic before deciding urgency, rather than act on code-reasoning alone.

**Delivered `scripts/check-silently-dropped-matches.ts` (read-only, no writes).** For every `search_matches` row already marked `surfaced_in_digest = true` with `surfaced_at >= 2026-09-05` (the date `getCadenceWindow()` — and therefore this exact silent-drop mechanism — first went live; nothing before that date is checked, since the prior behavior emailed everything unsurfaced regardless of age), it recomputes what that search's cadence window would have been at the moment `surfaced_at` was set, using the same `getCadenceWindow()` the real digest run used, and checks whether the company's `registration_date` actually fell inside that reconstructed window. A `registration_date` older than `windowStartDate` could only have been surfaced via the silent-drop branch — mechanically impossible to have been emailed. Reports counts of likely-emailed / silently-dropped / anomaly per cadence and overall, plus a drop-rate percentage. Answers one question only — what fraction of everything marked "surfaced" since this feature existed was actually emailed vs. silently never sent — and changes no data.

**Verified:** applied to the same sandbox clone, `npx tsc --noEmit` clean, `npx eslint scripts/check-silently-dropped-matches.ts` clean. Not yet run against production — asked the user to run `npx tsx scripts/check-silently-dropped-matches.ts` and share the output next.

**Note on interpreting the result:** because the underlying feature is only ~1-2 days old at the time this runs, absolute counts will be small regardless of severity — the drop-RATE is what matters for projecting forward, not the raw count from a short observation window. Also still open and unrelated to this specific script: Finding 1 from the previous entry (daily pipeline never captures business-type/商業 registrations at all) remains an undecided product-priority question for the user, not started.

No code files changed to fix anything in this entry — diagnostic only, and the density-gap question from the previous entry is now closed (confirmed characteristic of GCIS's own API, not a bug here). New file: `scripts/check-silently-dropped-matches.ts`.

```
git add scripts/check-silently-dropped-matches.ts architecture.md
git commit -m "Add diagnostic sizing potential silent, permanent loss of digest notifications for late-indexed GCIS registrations"
git push
```

## Correction: check-silently-dropped-matches.ts's first run was a bug in the script, not a real 100% drop rate — 2026-09-06, continued

User ran the script above against production immediately. Result: 3,460 rows checked, **100.0% landed in the "anomaly" bucket** for every cadence present (monthly 2,276, daily 1,184; no weekly rows in this window) — zero likely-emailed, zero silently-dropped. A uniform 100% anomaly rate across every row, with no exceptions, is itself a strong signal of a bug in the *script*, not a real property of the data — a genuine production issue would essentially never be perfectly uniform across a mix of independent rows. Did not treat this as a real finding; investigated before reporting it as one.

**Root cause, confirmed (not guessed):** the script compared `c.registration_date` (a SQL `DATE` column) directly against `windowStartDate`/`windowEndDate` (plain `"YYYY-MM-DD"` strings) using JS's `<`/`<=` operators. Grepped `@neondatabase/serverless`'s bundled type-parser table directly (`node_modules/@neondatabase/serverless/index.mjs`): DATE (OID 1082) is registered against its `parseDate` parser, confirming the driver returns DATE columns as native JS `Date` objects, not strings. Verified the actual failure mode with `node -e`: `new Date("2026-08-04T00:00:00.000Z") <= "2026-08-04"` evaluates to `false` — JS's abstract relational comparison converts the `Date` to its epoch-millis Number (via `valueOf()`) and the string via `ToNumber` (which yields `NaN` for a non-numeric string like `"2026-08-04"`), and any comparison against `NaN` is `false`. So both `<` and `<=` were unconditionally `false` for every row, regardless of the actual dates involved, which routes every row into the `else` (anomaly) branch — exactly reproducing the 100.0% result, deterministically, with no connection to real drop rates.

**Why this doesn't call the earlier density-gap or digest-window work into question:** grepped `lib/email/digest.ts` and the digest-export route directly — both compare `c.registration_date >= ${windowStartDate}` / `<= ${windowEndDate}` *inside the SQL query itself*, as query parameters Postgres casts and compares server-side. That comparison never touches JS's `<`/`<=` operators on a client-side `Date` object at all, so it was never exposed to this bug — the real digest send logic is unaffected. This bug was specific to this one retroactive diagnostic script, which chose to reconstruct the comparison in JS instead of SQL.

**Fix:** cast the column in the query itself — `c.registration_date::text AS registration_date` — so Postgres's own ISO `DateStyle` output (`YYYY-MM-DD`) reaches the JS side as an unambiguous string, never a `Date` object, regardless of what the driver's default type parser does. Also added a runtime guard immediately after the query: if the first row's `registration_date` isn't a plain string matching `/^\d{4}-\d{2}-\d{2}$/`, the script throws immediately with the actual value it saw, rather than silently producing another false 100%-anomaly result if this class of bug recurs after some future edit.

**Verified:** re-applied to the same sandbox clone, `npx tsc --noEmit` clean, `npx eslint scripts/check-silently-dropped-matches.ts` clean. Not yet re-run against production — asked the user to run it again; this time a real (not artifact) distribution across likely-emailed / silently-dropped / anomaly is expected.

**Modified:** `scripts/check-silently-dropped-matches.ts` (query casts `registration_date::text`; added a runtime type guard; no other logic changed).

```
git add scripts/check-silently-dropped-matches.ts architecture.md
git commit -m "Fix check-silently-dropped-matches.ts: DATE column was returned as a JS Date object, breaking its string comparisons and producing a false 100% anomaly rate"
git push
```

**Operational note, not a code change:** the device-file-delivery step for the corrected script silently failed the first time — the delivery tool reported success (no error, no rejection) but re-staging the file back from the device immediately after showed it was still the old, pre-fix content. The same thing happened to this very `architecture.md` file's own delivery in the entry above (first commit attempt "succeeded" but re-staging showed the pre-correction version; a second, identical commit attempt landed correctly). Net effect: the user re-ran the script believing it was fixed and got the identical false 100%-anomaly output a second time, purely because the fix was never actually on their machine yet. Re-delivered the script a second time and this time confirmed byte-for-byte (`diff` against the sandbox source) before telling the user to re-run. **Lesson for future sessions: after any `device_commit_files` call, always re-stage the same file and verify its actual content (not just the tool's "written" response) before treating a delivery as done** — this has now failed silently twice in one session and cannot be assumed reliable on its own.

## check-silently-dropped-matches.ts, second real run: 90.7% overall drop rate needed one more layer of scrutiny before it meant anything — 2026-09-06, continued

With the type bug fixed, the user's second run returned real (non-anomalous) numbers: 3,460 rows, 323 likely-emailed, 3,137 silently dropped, **90.7% overall silent-drop rate** (daily 98.2%, monthly 86.7%). Before reporting this as "90% of notifications are being lost," checked how `search_matches` rows actually get created, since a rate this extreme is exactly the kind of number that deserves a second look rather than being taken at face value (the same instinct that caught this script's own earlier bug).

**What the check-august-ingestion-gap and check-gcis-live-recount investigation hadn't accounted for:** `lib/matching/engine.ts`'s `matchSearch()` does a full re-scan of the *entire* `companies` table against a search's filter every time it runs (`SELECT uniform_id FROM companies WHERE <filters>`, then `INSERT ... ON CONFLICT (saved_search_id, company_uniform_id) DO NOTHING`) — it has no "only companies added since last run" scoping at all. Two things make that matter a lot for this specific number, both confirmed via `git log`:

1. `matchAllSearches()` was wired into the **monthly** ingestion back at Session 15 (`bb4d7f7`), but only wired into the **daily** ingestion on **2026-08-30**. Before that date, a daily-cadence search's `search_matches` could only be populated by the monthly run or a manual "Run Now" click — meaning its very first daily run on 8/30 necessarily did a full historical rescan for every active search at once, a one-time flood of matches against companies that had been registered for months.
2. Industry-code enrichment (Session 20b's 8/22 backfill, plus `retryRecentGaps()` running since 8/30) keeps making previously-`industry_codes = '{}'` companies newly satisfy a search's array-overlap filter well after their actual registration date — each case is *also* a fresh, first-time `search_matches` row for an old company.

Both mechanisms create search_matches rows where the company was never "fresh" at the moment it was first matched to that search — and `digest.ts`'s stale-match logic correctly declines to email a company that's already months old the first time it's matched, by design (see its own comment: waiting for a later run "would just grow an invisible backlog forever"). That's not a lost real-time notification; it's the intended behavior working correctly on backfill/enrichment-driven matches.

**Refined the diagnostic rather than accepting or discarding the 90.7% number.** `search_matches.matched_at` (confirmed via `db/schema.sql`: `TIMESTAMPTZ NOT NULL DEFAULT now()`) records when a match was actually created — comparing it against the company's own `registration_date` tells backfill/enrichment matches (matched long after registration) apart from genuinely prompt ones. Updated `scripts/check-silently-dropped-matches.ts` to add `sm.matched_at` and `c.source_dataset` to its query and split the "dropped" bucket into `droppedTimely` (matched within `TIMELY_MATCH_DAYS` = 7 days of `registration_date` — a real near-real-time capture that still got dropped) vs `droppedBackfill` (matched much later — expected, not a bug). The rate that actually answers the original question is `droppedTimely / (emailed + droppedTimely)`, excluding backfill matches from the denominator entirely. Also prints a `source_dataset` breakdown of just the timely-dropped rows, to see whether real drops concentrate in `gcis_daily_setup_query` (the near-real-time path) as the original hypothesis would predict.

**Verified:** re-applied to the sandbox clone, `npx tsc --noEmit` clean; `npx eslint` initially flagged one unused variable (`total`, left over from the previous version's bucket-sum logic that this refinement no longer needs) — removed it, re-ran clean. **Delivery required two attempts again** (first `device_commit_files` call reported success but re-staging showed the pre-refinement 6,574-byte file; second attempt landed the real 12,069-byte version) — confirmed byte-for-byte via `diff` against the sandbox source before telling the user to re-run a third time. This repeated delivery flakiness (three occurrences in one session now, across two different files) makes the "always verify by re-staging" rule from the entry above not just good practice but load-bearing.

**Modified:** `scripts/check-silently-dropped-matches.ts` (adds `matched_at`/`source_dataset` to the query; splits dropped matches into timely vs. backfill; no change to the underlying fixed date-comparison logic from the previous entry).

```
git add scripts/check-silently-dropped-matches.ts architecture.md
git commit -m "Refine check-silently-dropped-matches.ts to separate genuine near-real-time drops from expected backfill/enrichment-driven matches"
git push
```

## Investigation concluded: a real, specific gap for 方案C (daily) subscribers, sized at ~57% of timely matches — 2026-09-06, continued

User re-ran the refined script. Real numbers: **monthly cadence, 0.0% timely-drop rate (0 of 302)** — its 30-day window comfortably absorbs the matching latency this investigation is about, so this isn't a monthly-tier problem at all. **Daily cadence, 57.1% timely-drop rate (28 dropped of 49 timely-matched)** — all 28 dropped rows are `source_dataset = 'gcis_daily_setup_query'`, i.e. the near-real-time path, exactly as the mechanism below predicts. Overall across both cadences, only 8.0% (28 of 351) — but that blended number understates the real, tier-specific picture and shouldn't be the headline: this is a 方案C-specific issue, not a platform-wide one.

**Root cause, now precisely identified (not just hypothesized) by combining two already-confirmed facts from this same investigation:** (1) `scripts/check-gcis-live-recount.ts`'s result showed GCIS's own live `Company_Setup_Date` index keeps filling in more of a given day's registrations for days to weeks afterward (captured-on-day averaged 99.8, live-count-weeks-later averaged 156.2 across six test dates — roughly 36% of a day's eventual registrations are simply not visible yet the next morning). (2) The daily digest's own cadence window is only 2 calendar days wide (`daily: 2` in `CADENCE_LOOKBACK_DAYS`, per direct user instruction earlier this session, deliberately kept tight to avoid nuisance). When a company's registration only becomes visible in GCIS's index a few days after the fact — a `matched_at` still well within `TIMELY_MATCH_DAYS` (7 days), so not a backfill artifact by any reasonable definition — it can already have aged past the daily window's 2-day cutoff the moment it's finally captured and matched. **This is a real, structural mismatch between how long GCIS actually takes to fully index a day and how narrow the daily digest's own freshness window is — not a bug in any single line of code, and not the alarming "90% of all notifications lost" the unrefined number suggested, but a genuine, sized, tier-specific product gap affecting roughly half of the borderline-timing matches for the highest-paying tier.**

**Absolute sample is still small (28 rows)** — this feature is one day old — so 57.1% should be read as "a large share, of a small number, with a mechanism that's independently well-supported by the recount script," not as a statistically bulletproof rate yet. Worth re-checking again in a week or two once more days of data have accumulated, before making any bigger architectural call based on this number alone.

**Deliberately not fixed in this entry — this needs the user's decision, not a unilateral code change:** two directions were considered but neither was built:
1. **Fix at the ingestion end** — have `run-ingest-daily.ts`'s discovery query look back further than just "yesterday" and "today" (e.g., re-query the last 5-7 ROC dates' `Company_Setup_Date` every day, not just the two it checks now), so a registration GCIS was slow to index still gets captured into `companies` while it's still within the daily digest's 2-day window. `fetchAllForDate()` is already idempotent (`ON CONFLICT (uniform_id) DO UPDATE`) and cheap relative to GCIS's own daily volume, so this looks low-risk, but widens the daily job's GCIS API call volume by roughly that same multiple and hasn't been checked against whatever rate-limit headroom currently exists.
2. **Loosen the daily digest window itself** — directly conflicts with the user's own explicit instruction earlier this session ("only companies registered yesterday and the day before, or else it will be too much a nuisance"), so this direction is very unlikely to be the right call without the user explicitly revisiting that tradeoff.

Presented both directions plus the sized numbers to the user; no action taken pending their steer on priority and approach.

**Modified:** nothing (diagnostic-only entry; conclusion of the investigation started by the user's original August-density question).

## Fix chosen and implemented: daily discovery now looks back 5 extra days instead of just yesterday/today — 2026-09-06, continued

User chose direction 1 from the previous entry: fix at the ingestion end, leave the daily digest's own 2-day window untouched (it was a deliberate earlier instruction, not the source of the problem).

**`scripts/run-ingest-daily.ts` changes:**

1. Generalized `todayRocDate()` / `yesterdayRocDate()` into a single `rocDateForOffset(daysAgo: number)`. `main()`'s `dates` array is now built from a new `DISCOVERY_LOOKBACK_DAYS = 5` constant (today plus the last 5 days — 6 dates total, up from 2), so a registration GCIS was slow to index on the day it happened gets several more chances to be discovered before it would age out of the daily digest's 2-day window. 5 was chosen as a middle ground: comfortably inside the `TIMELY_MATCH_DAYS` (7) the diagnostic script already treats as "a genuine near-real-time capture," without trying to chase GCIS's index all the way out to the multi-week tail `check-gcis-live-recount.ts` observed (that tail remains the monthly bulk pipeline's job, unchanged).

2. **Without a second change, widening the lookback alone would have re-fetched full profile + industry-code data for every already-known company up to 6 times instead of 2** — `ingestDate()` previously always called `fetchProfile()` and `fetchLiveIndustryCodes()` for every discovered row regardless of whether it was already in the database, checking `existing` only afterward for the new-vs-updated count. Moved that `existing` check to the very top of the loop, before either expensive GCIS call, and `continue` immediately for an already-known company. This makes the lookback's added cost proportional to how many companies GCIS was actually slow to index (the thing this change exists to catch) rather than multiplying by `DISCOVERY_LOOKBACK_DAYS` — the extra cost of the wider window is now mostly just cheap discovery-list pagination for the 4 additional dates, not 4x the per-company profile/industry fetch volume.

3. **`updated_count` (the `ingestion_runs` column and `runStats.updatedCount` field) is repurposed, not renamed**, to avoid an `ingestion_runs` schema migration for this fix: it used to count companies that got their profile data re-fetched and re-upserted; now that already-known companies short-circuit before any of that happens, it counts "already known, skipped" instead. A run where this number is large now just means the lookback re-discovered a lot of already-captured companies (expected, not a problem) rather than that anything was actually updated. Log line and comments updated to match.

4. The `ON CONFLICT (uniform_id) DO UPDATE` branch on the `companies` INSERT is now normally dead code (the `existing` check above already prevents reaching it for a known company) — left in place only as a safety net for the rare race between that `SELECT` and this `INSERT` (e.g. two overlapping manual `workflow_dispatch` runs), not as an expected path.

**Side effect worth naming explicitly:** before this change, an already-known company effectively got its profile re-checked on two separate days (once as "today," again the next day as "yesterday" under the old 2-date design), which incidentally gave the daily job a small, opportunistic chance to notice a status change (e.g. dissolution) within its lookback window. Skipping already-known companies removes that opportunistic re-check. This isn't a regression in correctness — the monthly bulk `company_change`/`company_dissolve` datasets (via `lib/ingestion/upsert.ts`) remain the authoritative, complete path for status transitions across the whole company base, exactly as they already were for any company outside the daily job's short lookback window — it just means status changes for very recently registered companies may take slightly longer to reflect if they happen to change status days after registering, before the next monthly run. Not expected to matter in practice (a company's status almost never changes within days of its own registration), but worth knowing about if it ever comes up.

**Verified:** `npx tsc --noEmit` clean, `npx eslint scripts/run-ingest-daily.ts` clean, on the same sandbox clone. Could not test against the real GCIS API or production database from this sandbox (no network access to `data.gcis.nat.gov.tw`, and this is a live-effect script, not read-only — unlike this session's other diagnostics, this one writes to `companies` and `ingestion_runs` for real). Delivered to the user's machine and **confirmed byte-for-byte via `diff` on the first delivery attempt** (no repeat of the delivery-tool flakiness from earlier entries this time). **Not yet run against production** — the next scheduled `ingest-daily.yml` run (22:00 UTC / 06:00 Taipei), or a manual `workflow_dispatch`, is what actually confirms this in practice. Worth watching the next run's duration against the workflow's existing 20-minute timeout, though the `existing`-check optimization above should keep the added lookback cheap (mostly extra discovery-list pagination, not extra per-company GCIS calls) — no timeout increase made preemptively; revisit if a run ever approaches it.

**Modified:** `scripts/run-ingest-daily.ts` (see above; no schema change, no workflow YAML change).

```
git add scripts/run-ingest-daily.ts architecture.md
...
## Diagnostic re-run today shows unchanged 57.1% daily-cadence drop rate — explained, not a fix failure — 2026-09-07

User re-ran `scripts/check-silently-dropped-matches.ts` same-day (sooner than the "few days" originally suggested). Result: identical numbers to the pre-fix baseline — daily cadence 21 emailed / 28 dropped-timely (57.1%), overall TIMELY drop rate 8.0%. Worth recording precisely why this is expected and not a sign the fix isn't working, so this doesn't get mistaken for a third false alarm in this saga:

`surfaced_at` (what the script's `WHERE sm.surfaced_at >= '2026-09-05'` filters on) is set by `digest.ts` at digest-send time, not at ingest/match time. The fix's first-ever production ingest run was #30 (commit `0b79bee`, ran 2026-09-06 23:27 UTC — see previous entry). For that run's `matchAllSearches()` output to show up as emailed-vs-dropped in this diagnostic, a digest.yml run has to execute *after* 23:27 UTC and process it. Checked `digest.yml`'s run history directly: the most recent run is still #10, triggered 2026-09-06 00:20 UTC — i.e. the digest cycle for 2026-09-05's ingestion, entirely before the fix existed. As of this entry (2026-09-07 00:10 UTC) tonight's digest run (for the 2026-09-06/fix cycle) had not yet fired. So literally zero rows in the current dataset could reflect the fix either way — the unchanged number is exactly what you'd expect from asking the question one cycle too early, not evidence of anything.

**Worth watching, not fixing yet:** `digest.yml`'s cron (23:00 UTC) is deliberately offset 1 hour after `ingest-daily.yml`'s (22:00 UTC) specifically so `matchAllSearches()` finishes before digest reads `search_matches` (see digest.yml's own schedule comment, added 2026-08-30). But both jobs are subject to GitHub Actions' own scheduling slip — tonight ingest fired 87 minutes late (23:27 UTC) and digest run #10 fired 80 minutes late for its own cycle (00:20 UTC the next day) — so the 1-hour buffer is not a hard guarantee under real-world slip, just usually enough. `digest-watchdog.yml` (added 2026-09-03) only catches "digest didn't run at all," not "digest ran before ingest's matchAllSearches() finished and read stale search_matches" — that specific ordering failure mode has no alerting today. Not proposing a fix for this now (no evidence it has ever actually happened), just flagging it as a latent gap for whoever next touches this scheduling.

**Next real check:** once `digest.yml` shows a run newer than #10 (confirms it processed the 2026-09-06/fix ingestion cycle), re-run `scripts/check-silently-dropped-matches.ts` again — that's the first data point that can actually move.

**Modified:** nothing (diagnostic-interpretation entry).

## Homepage pricing teaser cards (方案B/C) made clickable straight into /pricing, with an always-visible upgrade CTA — 2026-09-07

Direct user request, alongside asking about 藍新 (NewebPay) status (see below): make upgrading to a paid plan more prominent/enticing on the homepage. Previously the homepage's pricing teaser section (`app/(marketing)/page.tsx`) rendered all three plans as plain, unclickable `<div>`s — the only way to actually reach `/pricing` from that section was one small "查看完整方案內容與功能比較 →" link below the whole grid.

**What changed:** the 方案B and 方案C teaser cards are now full `<Link>`s to `/pricing#plan-b` and `/pricing#plan-c` respectively (matching `id` attributes + `scroll-mt-8` added to those two cards on the actual `/pricing` page, so the visitor lands on the specific plan they clicked rather than the top of a page with three plans to re-scan). Both cards gained a hover shadow-lift and an always-visible "立即升級 →" line in accent color under the price — deliberately NOT hover-only, since a hover-revealed CTA would be permanently invisible on mobile (no hover state), and mobile is a realistic share of this site's traffic. 免費方案's card was deliberately left as a plain, non-clickable div — nothing to "upgrade" to there, and the user's own ask was specifically about the paid plans.

**Scope note:** this only makes the homepage teaser more clickable and the destination easier to land on correctly — it does not touch `/pricing` itself beyond adding the two anchor ids, and it does NOT change or work around the NewebPay-not-configured-yet situation below. Every visitor this drives to `/pricing#plan-b`/`#plan-c` still hits the same real `NewebpayCheckoutButton` that currently fails with "NewebPay 尚未設定完成" on click, same as before this change — see the entry immediately below. Flagged to the user directly before building; she wanted this shipped regardless, since it's still a real improvement to the funnel up to that point.

**Verified:** fresh `npm ci` + `npx next typegen` in an isolated sandbox clone, `npx tsc --noEmit` clean, `npx eslint` clean on both changed files, full `npx eslint .` showing the exact same 10 pre-existing unrelated errors as every prior round this project (none new, none in these files). Delivered to the user's machine and confirmed byte-for-byte identical via `diff` on the first attempt.

**Modified:** `app/(marketing)/page.tsx`, `app/(marketing)/pricing/page.tsx`.

```
git add "app/(marketing)/page.tsx" "app/(marketing)/pricing/page.tsx" architecture.md
git commit -m "Make homepage pricing teaser cards (方案B/C) clickable into their matching /pricing plan, with an always-visible upgrade CTA"
git push
```

## Status check: 藍新 (NewebPay) integration is actually mostly BUILT, just not connected to a real account yet — 2026-09-07

User asked directly whether 藍新 implementation was still undone. Re-read the actual code (not just architecture.md) to answer precisely rather than from memory, since this spans several sessions' worth of work:

**What's already built and live on the site today:** the full checkout UI switched from Paddle to `NewebpayCheckoutButton` on both `/pricing` and the account page back on 2026-09-05 — Paddle is hidden from every surface (though its code is untouched and still technically functional, just unreachable). Two genuinely different NewebPay products are wired in: monthly billing goes through the 信用卡定期定額 (Period, recurring, card-only) API; yearly billing goes through a separate one-time MPG checkout that actually offers ATM transfer/超商代碼/barcode alongside card (Period can't do this — it's card-only). Both webhook handlers exist (`app/api/webhooks/newebpay/route.ts`, `.../newebpay-mpg/route.ts`), the DB schema has the NewebPay-specific columns and a `newebpay_pending_orders` table, cancellation (`alterNewebpayPeriodStatus()`) and tier-resolution (`getUserTier()`) are both already processor-agnostic (handle Paddle or NewebPay subscriptions identically), and the legally-required business-use checkbox (2026-09-06) gates both checkout routes server-side, not just client-side.

**What's actually missing — the one real gap:** `NEWEBPAY_MERCHANT_ID`, `NEWEBPAY_HASH_KEY`, `NEWEBPAY_HASH_IV`. No real 藍新 merchant account exists yet, so these three env vars are unset everywhere (and — noticed while checking `.env.example` for this — that file is stale and doesn't even list them, since it predates the 2026-09-04/05 NewebPay build; worth adding them there at some point as housekeeping, not done in this entry). Every checkout route calls `requireEnv()` internally and fails with a clean 503 ("NewebPay 尚未設定完成，目前無法使用此付款方式") rather than crashing — this was built deliberately "looks and behaves like the final feature, fails safely at the one point that needs real credentials" (2026-09-05 entry) so it's safe to leave live. **Concretely: right now, literally no one can successfully subscribe to Plan B or C** — every checkout click dead-ends at that error, confirmed by re-reading `/pricing/page.tsx`'s own header comment.

**What's needed from 藍新 to actually finish this, per this project's own prior research (2026-09-04 entries):**
1. Apply for and get approved for a 藍新 merchant account — decided as **個人 (individual, unincorporated)**, not 企業, per the 2026-09-04 decision (ties to the still-separate incorporation question; 個人 caps at NT$200,000/transaction, no 統一編號 needed, vs. 企業's NT$600,000 cap requiring one).
2. NewebPay's own **商店管理規範**, confirmed by directly visiting their registration page, requires the applicant's site to already look fully live and legitimate *before* applying — real pricing (have it), a real description of what's sold (have it), visible customer-service contact info that matches what's declared to NewebPay (have email; no phone number is shown anywhere on the site today — only matters if a phone number is also given at registration), a privacy policy (have it, published 2026-09-04), and critically, **zero "測試"/placeholder/demo language anywhere on the live site** plus the displayed site name matching whatever store name gets registered. This requirement is about the site's *content and business legitimacy*, not about the checkout code being wired in first — that step correctly comes after approval, since the real credentials don't exist until then.
3. Once approved, 藍新 issues the three credentials above — set them in Vercel's production env (and `.env.local` for local dev), and this integration goes live without any further code changes on the happy path.
4. **Before trusting this with real money**, test end-to-end against a real or sandbox NewebPay account. Every field name/format in `lib/newebpay-api.ts` (periodPoint's exact encoding, ProdDesc's charset limits, the notify-envelope field names both webhooks expect) was reverse-engineered from third-party integration writeups and NewebPay's own older PDF (NDNP-1.0.4) — the newest official spec (NDNP-1.0.6) could not be fetched this project (NewebPay's own download links 404'd/were session-gated). Nothing in this integration has been run against a live or sandbox NewebPay endpoint even once.

**Current sequencing decision (2026-09-05, still standing, reaffirmed by not being revisited today):** finish the rest of the site to a genuinely complete, presentable state first, then apply to 藍新, then do the end-to-end sandbox testing, then cut over for real. Per the user's standing instruction (see this file's own top-of-conversation ground rules): do not re-raise "go live with Paddle in the meantime" — that's already been explicitly and repeatedly declined.

**Modified:** nothing (status-check entry, no code touched).

## Terms/Privacy updated to name 藍新 (NewebPay) instead of Paddle; cancel/refund audit found nothing missing but one real stale claim fixed — 2026-09-07

Direct user request, made mid-way through the 藍新 registration walkthrough: update ToS to reflect 藍新, and check whether the site has a working cancel/refund function, adding one if missing.

**Found: Paddle was still named in FOUR live user-facing spots, not just one.** A full grep of `app/` for "Paddle" outside code comments turned up Terms 第四條 (payment processor) and 第五條 (upgrade/downgrade proration), plus two spots in Privacy Policy that hadn't been touched in any prior NewebPay-switch entry: 二、我們蒐集的個人資料 (payment info collection) and, more significantly, 八、接收者與第三方服務提供者 and 九、國際傳輸 (the third-party processor list and cross-border transfer disclosure).

**Straightforward fixes (mechanical, low-risk):** Terms 第四條's processor line, and Privacy 二's payment-collection line, both now name 藍新科技股份有限公司（藍新金流）instead of Paddle — purely factual, who actually processes the payment today.

**Not just a name swap — flagging clearly, not a lawyer, do not treat as legal advice:** Privacy 八's original Paddle entry specifically described Paddle as "Merchant of Record（收單機構），其法律上係以自己名義與買方訂立交易契約" — Paddle contracts with the customer in its own name, a materially different legal role than a payment gateway. 藍新's 個人 (individual) merchant product is NOT a Merchant of Record arrangement — the user, as the registered individual merchant, is the actual contracting party with subscribers; 藍新 just moves the money. Rewrote that entry to reflect this (藍新 as payment processor only, contract remains between subscriber and 新公司快報), and correspondingly removed 藍新 from 九、國際傳輸's cross-border-transfer disclosure (藍新 is a Taiwan company — domestic, not an overseas transfer the way UK-based Paddle was), leaving only Neon/Vercel/Resend (all US) as the actual overseas transfers. This is a genuine, substantive change to what the Privacy Policy legally asserts, not a mechanical correction — this document was previously lawyer-reviewed and published "as-is, no changes" (2026-09-04 entry). **Recommend the user have her lawyer glance at Privacy 八 and 九 specifically** before treating this as final; everything else in this round is a plain factual swap that doesn't need that.

**Real functional gap found and fixed in Terms, not invented:** Terms 第五條 claimed "訂閱方案得隨時升級或降級，差額將依 Paddle 之比例計算方式處理" (can change plans anytime, Paddle handles proration) — but re-reading `app/api/account/change-plan/route.ts` and `AccountPageClient.tsx` (see this file's 2026-09-05 "hide Paddle from the surface" entry) confirmed the change-plan button was **already deliberately removed from the account page UI** that same day, specifically because it only ever called Paddle's API and has no NewebPay equivalent — this was a known, already-decided tradeoff, not new. So the claim was doubly wrong: wrong processor AND describing a self-service feature that doesn't currently exist for anyone. Rewrote it to match reality: changing plans currently means emailing contact@taiwanleads.com, with a line reserving the right to add self-service switching later. This is an honest correction to match already-shipped product behavior, not a new business decision — didn't ask before making it.

**Cancel/refund audit: nothing was missing.** `app/api/account/cancel/route.ts` already exists, already handles both Paddle and NewebPay subscriptions identically (confirmed by re-reading it directly), and `AccountPageClient.tsx` already renders a working 取消訂閱 button wired to it (correctly hidden only for the one-time yearly MPG purchase, which has no recurring commitment to cancel). No monetary refund mechanism exists, but that's a disclosed, deliberate policy already stated in Terms 第六條 (cancel anytime, keep access until the already-paid period ends, no refund implied) and echoed on `/pricing`'s trust-signal line — not an oversight, and not something built silently this round. Told the user this is already satisfied rather than adding a redundant feature or policy statement on my own judgment; a real monetary-refund capability would be a business/legal decision for her to make explicitly, not something to build unprompted.

**Verified:** fresh `npm ci` + `npx next typegen` in an isolated sandbox clone, `npx tsc --noEmit` clean, `npx eslint` clean on both changed files, full `npx eslint .` showing the same 10 pre-existing unrelated errors as every prior round (none new). Delivered to the user's machine and confirmed byte-for-byte identical via `diff` on the first attempt.

**Modified:** `app/(marketing)/terms/page.tsx`, `app/(marketing)/privacy/page.tsx`.

```
git add "app/(marketing)/terms/page.tsx" "app/(marketing)/privacy/page.tsx" architecture.md
git commit -m "Update Terms and Privacy Policy to name 藍新 (NewebPay) instead of Paddle; fix stale upgrade/downgrade claim to match already-removed change-plan UI"
git push
```

## Built self-service NewebPay plan-switch (Plan B ↔ Plan C, monthly Period subscribers) — 2026-09-08

Direct follow-up to the previous entry's Terms 第五條 correction, which admitted plan-switching currently means emailing support. User asked "they can not upgrade by pushing a button?", was told no — `app/api/account/change-plan/route.ts` is Paddle-only and NewebPay's Period API has no "change the amount on an existing commitment" endpoint at all, only create (new authorization) and AlterStatus (restart/suspend/terminate an existing one) — and, offered the choice between building it now or leaving email-support as the answer given how few subscribers exist at this stage, said yes, build it.

**Design constraint that shaped everything below:** because NewebPay Period has no in-place "change price" call, a plan switch is unavoidably *create a brand-new Period commitment at the target tier's full price, then terminate the old one* — not the other way round, and not a single atomic operation. The one failure mode worth designing around deliberately: a customer abandons the new checkout (closes the NewebPay tab, declines the card prompt) after the old commitment has already been terminated — they'd be left with zero active subscription, entirely due to a UI flow, not anything they did wrong. Fixed by strict sequencing: the OLD commitment is only ever terminated from inside the webhook handler, after the NEW commitment's payment SUCCESS notify has already been confirmed — never at checkout-initiation time. An abandoned switch just leaves the customer exactly where they started.

**Five pieces, in dependency order:**

1. **`scripts/migrate-add-supersedes-period-no.ts`** (new) — idempotent `ALTER TABLE newebpay_pending_orders ADD COLUMN IF NOT EXISTS supersedes_period_no TEXT`, same pattern as `migrate-add-business-use-confirmation.ts`. Also documented directly in `db/schema.sql`'s `newebpay_pending_orders` table so a fresh setup gets the column without needing to know this migration exists. NULL for every ordinary new-purchase pending order; only the new switch route below sets it.

2. **`app/api/checkout/newebpay-switch/route.ts`** (new) — the checkout-initiation route for an *existing* monthly NewebPay Period subscriber. Validates: signed in, `targetTier` is `pro`/`business` and differs from current tier, the subscriber's most-recent active (non-expired) subscription actually has a `newebpay_period_no` (rejects Paddle subscribers — they have `change-plan` — and one-time yearly MPG buyers — no recurring commitment exists for this to supersede), the business-use checkbox is checked (this is a brand-new charge, so Terms 第六條's confirmation applies exactly as it does to a first purchase), and there's no other unclaimed pending order already in flight for this user (guards a double-click from creating two competing new commitments). Builds a fresh `buildCreatePeriodOrderRequest()` at the target tier's full monthly price — today's day-of-month as the new billing point, 99 cycles, same unconfirmed-format caveats as the original checkout route — and inserts a `newebpay_pending_orders` row recording `supersedes_period_no = <the old period no>`. Returns the same `{url, postData, merchantId}` shape the original checkout route does, so the button component below can reuse the exact same `buildFormAndSubmit()` redirect pattern.

3. **`app/api/webhooks/newebpay/route.ts`** (modified) — the pending-order SELECT now also reads `supersedes_period_no`. After the new subscription row is inserted and the pending order is claimed (existing logic, unchanged), a new block runs only when `supersedes_period_no` is set: calls `alterNewebpayPeriodStatus(oldPeriodNo, "terminate")`, and **only if that call reports success**, marks the old `subscriptions` row `status = 'canceled', canceled_at = now()`. Deliberately does NOT mark the old row canceled on a failed/thrown AlterStatus call — see the code comment: this app should never claim a commitment was terminated when NewebPay hasn't actually confirmed that, since a customer's own tier resolution (`getUserTier()`'s "most recent active row wins" query) already resolves correctly to the new plan regardless of whether the old row's status gets updated, so there's no urgency to lie about it just to tidy the UI. **Known, explicitly flagged operational gap: if AlterStatus fails here, the only signal is a `console.error` with "MANUAL INTERVENTION REQUIRED" in the message — nothing pages anyone, retries, or surfaces this in any admin UI.** Someone has to notice it in production logs (or catch it by eyeballing NewebPay's back office) to avoid a customer being double-billed on both the old and new commitments simultaneously. Worth building real alerting for this before this feature sees meaningful volume — not done here, given the current subscriber count.

4. **`app/api/account/route.ts`** (modified) — every response branch now includes an explicit `processor: "paddle" | "newebpay_period" | "newebpay_yearly" | null` field. Needed because `tier`/`autoRenew` alone can't tell a Paddle subscriber apart from a NewebPay Period one (both report a real paid tier and `autoRenew: true`) — that ambiguity never mattered before (cancel already worked identically for both), but the new switch button only works for `newebpay_period`, so the client needs a real signal to gate on instead of guessing.

5. **`components/NewebpaySwitchPlanButton.tsx`** (new) + **`AccountPageClient.tsx`** (modified) — a new "變更方案" card, shown only when `tier !== "free"`, `processor === "newebpay_period"`, and not already `scheduledCancellation` (switching plans on a subscription that's already ending would just create a second commitment on top of someone in the middle of leaving). Renders a single button toward whichever plan isn't the current one ("升級至方案 C" from pro, "降級至方案 B" from business — cosmetic labeling only, both directions behave identically server-side). Clicking expands an inline confirmation with the same business-use checkbox as first-time checkout and explicit copy stating this is a full new charge at the target tier's price with **no proration or credit for the old plan's unused time** — a real, deliberate simplification (matching the "safety over billing elegance, given how few subscribers exist" framing from this conversation) over building proration logic against a payment integration that has never once been tested against a real NewebPay endpoint.

**Standing caveat, same as every other file in this integration:** nothing here has been tested against a real or sandbox NewebPay account — `NEWEBPAY_MERCHANT_ID`/`HASH_KEY`/`HASH_IV` still don't exist. This route fails the same clean 503 the original checkout route does until a real merchant account exists, so it's safe to ship live now and verify for real once 藍新 approval comes through.

**Not done, worth revisiting once this is verified working:** Terms 第五條 currently says self-service switching doesn't exist yet ("請透過客服信箱...本服務保留日後提供線上自助變更功能之權利") — should be updated to describe this feature once it's confirmed working against a real account, not before (no point promising a button that's never actually processed a real payment).

**Verified:** fresh `npm ci` + `npx next typegen` in an isolated sandbox clone (`/tmp/tlr`, re-cloned this session), `npx tsc --noEmit` clean, `npx eslint` clean on all five new/changed files, full `npx eslint .` showing the same 10 pre-existing unrelated errors as every prior round (confirmed by file — `AccountPageClient.tsx`'s one pre-existing error is in the untouched `loadAccount()`/`useEffect` block, not anything added here).

**Modified/added:** `scripts/migrate-add-supersedes-period-no.ts` (new), `db/schema.sql`, `app/api/checkout/newebpay-switch/route.ts` (new), `app/api/webhooks/newebpay/route.ts`, `app/api/account/route.ts`, `components/NewebpaySwitchPlanButton.tsx` (new), `app/(app)/account/AccountPageClient.tsx`.

```
git add scripts/migrate-add-supersedes-period-no.ts db/schema.sql "app/api/checkout/newebpay-switch/route.ts" "app/api/webhooks/newebpay/route.ts" "app/api/account/route.ts" "components/NewebpaySwitchPlanButton.tsx" "app/(app)/account/AccountPageClient.tsx" architecture.md
git commit -m "Add self-service NewebPay plan-switch (Plan B <-> Plan C) for existing monthly Period subscribers, with terminate-after-confirm sequencing to avoid a zero-subscription failure mode"
git push
```
